...
<div class="Wrap"><!--// 전체를 감싸는 Wrap -->
		...
		<button onClick="layerPop('ifm_reg','./donation2_reg.php', '100%', 650)"; class="btnSearch ">후원금 등록</button>
		</div>
							...
							<?
								...
								if($data_list_cnt > 0 ) {
									for ($i = 0 ; $i < $data_list_cnt ; $i++) {
                                        ...
                                        $do_select      = $data_list[$i]['do_select'];
                                        $do_gubun      = $data_list[$i]['do_gubun'];
                                        $do_pay      = $data_list[$i]['do_pay'];
										$do_type       = $data_list[$i]['do_type'];										
										$do_show       = $data_list[$i]['do_show'];										
										$do_regdate    = $data_list[$i]['do_regdate'];																			
																				
										$edit_btn = $C_Button -> getButtonDesign('type2','수정',0,"layerPop('ifm_reg','./donation2_edit.php?do_idx=" . $do_idx. "', '100%', 650)", 50,'');	
										$edit_btn .= $C_Button -> getButtonDesign('type2','삭제',0,"donation2_delete('" . $do_idx. "')", 50,'');
							?>
									<tr>
                                        <td><?=$data['page_info']['start_num']--?></td>									
										<td><?=$do_year?></td>
										<td><?=$GP -> donation_TYPE[$do_select]?></td>
                                        <td><?=$do_gubun?></td>	
...
<script type="text/javascript">
	...
	function donation2_delete(do_idx)
	{
		...
		$.ajax({
			...
			data: "mode=donation2_DEL&do_idx=" + do_idx,
			dataType: "text",
