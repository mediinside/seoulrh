...
switch($_POST['mode']){
	...
	case "DOCTOR_MODI" :
		...
		//사진 업로드
		$file_orName			= "image_profile_list";
		$is_fileName			= $_FILES[$file_orName]['name'];
		$insertFileCheck	= false;
		if ($is_fileName) {
			$args_f = "";
			...
			if ($updata['error']) $insertFileCheck = true;			
				$image_m_profile_img = $updata['new_file_name'];	//변경된 파일명
		}else {
			$image_m_profile_img				= $before_m_profile_img;
		}
		...
		$args['dr_m_profile_img'] = $image_m_profile_img;
	...
	case "DOCTOR_REG":
		...
		if ($is_fileName) {
			...
			if ($updata['error']) $insertFileCheck = true;			
				$image_m_profile = $updata['new_file_name'];	//변경된 파일명
		}else {
			$image_m_profile				= $before_image_m_list2;
		}
		...
		$args['dr_list_img']     = $image_list;
		$args['dr_m_list_img']   = $image_m_list;
		$args['dr_m_profile_img']  = $image_m_list2;
		$args['dr_vod_thum_img'] = $image_vod_thum_list;
		$args['dr_main_view']    = $dr_main_view;	
		$args['dr_vod_url']      = $dr_vod_url;	
		$args['dr_history']      = $C_Func->enc_contents_edit($dr_history);		
