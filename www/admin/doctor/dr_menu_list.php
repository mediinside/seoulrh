<?php
	...
	$args['show_row'] = 50;
    $args['pagetype'] = "admin";
    $args['dr_menu'] = $_GET["dr_menu"] ;
	$data = "";
	// echo $_SESSION['mb_branch'];
	if($_SESSION['adminid'] == 'medi2'  || $_SESSION['adminid'] == 'anyspine') {
		$args['dr_branch'] = 'anyang';
		$dr_branch = 'anyang';
	}
...
<div class="Wrap"><!--// 전체를 감싸는 Wrap -->
		...
		<!--div class="btnWrap" style="display: none;"-->
			...
			<button onClick="layerPop('ifm_reg','./dr_menu_reg.php?dr_menu=<?=$_GET["dr_menu"]?>', '100%', 600)"; class="btnSearch btnRight">의료진 등록</button>
		</div>
						...
						<tbody>
							...
							<?
								...
								for ($i = 0 ; $i < $data_list_cnt ; $i++) {
									...
									$dr_m_list_img		= $data_list[$i]['dr_m_list_img'];	
									$dr_vod_thum_img		= $data_list[$i]['dr_vod_thum_img'];	
									...
									$dr_img2 = '';
									if($dr_m_list_img !=  '') {
										$dr_img2 = "<img src='" . $GP -> UP_DOCTOR_URL . $dr_m_list_img . "' width='100px' />";
									}

									$dr_img3 = '';
									if($dr_vod_thum_img !=  '') {
										$dr_img3 = "<img src='" . $GP -> UP_DOCTOR_URL . $dr_vod_thum_img . "' width='100px' />";
									}
									
									$book_btn = $C_Button -> getButtonDesign('type2','관리',0,"layerPop('ifm_reg','./book_list.php?dr_idx=" . $dr_idx. "', '100%', 800)", 50,'');									
									$edit_btn = $C_Button -> getButtonDesign('type2','수정',0,"layerPop('ifm_reg','./dr_menu_edit.php?dr_idx=" . $dr_idx. "', '100%', 800)", 50,'');	
								...
								?>
										<tr id="<?=$dr_idx?>">
											...
											<td><?=$dr_img2?></td-->
											<td><?=$dr_img3?></td>
											<td><?=$GP -> CENTER_TYPE[$dr_center]?></td>
