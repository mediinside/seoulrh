						...
						<tbody>
							...
							for ($i = 0 ; $i < $data_list_cnt ; $i++) {
								...
								$jb_code 					= $data_list[$i]['jb_code'];
									...
									$write_btn = $C_Button -> getButtonDesign('type2','글쓰기',0,"layerPop('ifm_reg','/bbs/index.php?jb_code=" . $jb_code. "&jb_mode=twrite', '100%', 1500)", 200,'');
								
									$edit_btn = $C_Button -> getButtonDesign('type2','관리',0,"javascript:BoardAdminWin(" . $jb_code .")", 50,'');
								
									if ($_SESSION['suserlevel'] == '9') {
										$edit_btn .= $C_Button -> getButtonDesign('type2','수정',0,"layerPop('ifm_reg','./bbs_edit.php?jb_code=" . $jb_code. "', '100%', 1100)", 50,'');
									}
							...
							?>
									<tr>
										...
										-->
										<td><?=$jba_skin_dir?></td>  									        
										<td style="text-align: center;"><?=$write_btn?><?=$edit_btn?></td>
									</tr>
									<?
								$dummy++;
							}
...
function BoardAdminWin(jb_code)
{
	...
	window.open (url,'board_admin_win' ,'scrollbars=yes, top=0, left=0, width=1100, height=900')
}
</script>