...
class MY_Kcaptcha {
	...
	function image() {
		...
		// 폰트 디렉토리의 png 파일을 모두 폰트로 읽어들임 (단, '_'로 시작하는 png는 제외함)
		$fonts=array();
		$fontsdir_absolute = APPPATH.'libraries/'.$fontsdir;
		if ($handle = opendir($fontsdir_absolute)) {
			while (FALSE !== ($file = readdir($handle))) {
				if (preg_match('/^[^_].*\.png$/i', $file)) {
		...
		// 새로고침 배경
		$refresh_bg = imagecolorallocatealpha($img2, $foreground_color[0], $foreground_color[1], $foreground_color[2], 60);
		imagefilledrectangle($img2, $width, $height - 12, 0, $height, $refresh_bg);
		// 텍스트
		imagettftext($img2, 7, 0, $width - 38, 47, $background, $fontsdir_absolute .'/NanumGothic.ttf', '새로고침');
		// 아이콘
		$icon = imagecreatefrompng($fontsdir_absolute .'/_icon_refresh.png');
		imagealphablending($img2, false);
		imagesavealpha($img2, true);
		
		$this->imagecopymerge_alpha($img2, $icon, $width - 51, $height - 12, 0, 0, 12, 12, 60);
