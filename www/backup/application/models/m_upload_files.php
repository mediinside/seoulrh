...
class M_upload_files extends CI_Model {
	...
	function is_board($table='') {
		if($table)
			return $this->db->where(array('bid' => $table))->count_all_results($this->M_basic->table);
		else
			return $this->M_basic->table;
	}
	...
	function form_upload($uf_table, $id, $file) {
		...
		foreach($file['tmp_name'] as $key => $val) {
			if($val) {
				...
				$filename = fileupload2($tfile, $dirs['data_path']);
