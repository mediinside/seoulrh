...
class M_banner extends CI_Model {
	...
	function get_banner($id='', $html_ret=TRUE) {
		...
		$this->db->where("(bn_edate >= '". TIME_YMDHIS ."' OR bn_edate = '0000-00-00 00:00:00')");
	...
	function html_banner($gr_info, $banner) {
		...
		foreach($banner AS $bn) {
			$img = "<img src='". $img_path ."/". $bn["bn_image"] ."' $imgW $imgH/>";
			$banner_html = $bn["bn_url"] ? '<a href="'. $bn["bn_url"] .'" target="'. $bn["bn_target"] .'">'. $img .'</a>' : $img;
			$html_code .= "<li class='$li_class'>$banner_html</li>\n";
		}
		$html_code .= "</ul></div>\n";
