...
class M_editor extends CI_Model {
	...
	// 파일 업로드
	function uploadFile($uf_table, $uf_id, $wr_content='', $is_editor=1, $editorNo=FALSE) {
		...
		if(is_numeric($editorNo)) {
			$images = $images[$editorNo];
			$inames = $inames[$editorNo];
			$files = $files[$editorNo];
			$fnames = $fnames[$editorNo];
		}
		...
		$result = $this->M_upload_files->get_files($uf_table, $uf_id, 'uf_no,uf_file', array('uf_editor'=>$is_editor));
		...
		if ($cont_file) {
			...
			if ($val_file = substr($val_file, 0, -1)) {
				...
				$this->M_upload_files->file_insert($val_file);
				$is_img[0] = $mime[0];
