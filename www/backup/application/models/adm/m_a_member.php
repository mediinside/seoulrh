...
class M_a_member extends CI_Model {
	...
	function list_result($sst, $sod, $sfl, $stx, $limit, $offset, $fields='') {
		$this->db->start_cache();
		if ($stx) {
			switch ($sfl) {
				case "mb_point" :
		...
		$this->db->select(setValue('mb_id,mb_name,mb_nick,mb_level,mb_point,mb_mailling,mb_open,mb_email_certify,mb_today_login,mb_leave_date', $fields));
		$this->db->order_by($sst, $sod);
		$this->db->where('mb_level <=', $member['mb_level']);
		$qry = $this->db->get($this->table, ($limit?$limit:NULL), $offset);
		$result['qry'] = $qry->result_array();

		$this->db->where('mb_leave_date >', '0');
	...
	function setConfig($sql) {
		...
		if($this->db->count_all_results($this->table_conf) > 0) {
			$this->db->update($this->table_conf, $sql);
		}
		else {
			$this->db->insert($this->table_conf, $sql);
		}
