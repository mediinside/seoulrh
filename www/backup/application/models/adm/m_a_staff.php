<?php
class M_a_staff extends CI_Model {
	var $table = 'ki_staff';
	...
	function record($w, $sql) {
		...
		if ($w == '') {
			$sql['st_regdate'] = TIME_YMDHIS;
			$this->db->insert($this->table, $sql);
			return $this->db->insert_id();
		}
		else {
			$this->db->update($this->table, $sql, array('st_id' => $this->input->post('st_id')));
			return $this->input->post('st_id');
		}
	}
	...
	function list_update($ids, $data) {
		...
		foreach ($ids as $key => $id) {
			...
			$batch[$key]['st_mdydate']	= TIME_YMDHIS;
