...
class View extends Widget {
	function index() {
		...
		if (!$board['bo_use_list_view']) {
			...
			// 이전글 링크
			$wr_prev = array('href' => '', 'subject' => '이전글이 없습니다.', 'html' => '<a>이전글이 없습니다.</a>');
			if ($pn['prev']['wr_id']) {
				$prev_wr_subject = cut_str(get_text($pn['prev']['wr_subject']), 255);
				$wr_prev['href'] = RT_PATH."/board/".BO_TABLE."/view".qstr_rep($qstr, 'wr_id', $pn['prev']['wr_id']);
				$wr_prev['subject'] = $prev_wr_subject;
				$wr_prev['html'] = '<a href="'. $wr_prev['href'] .'">'. $wr_prev['subject'] .'</a>';
			}
			
			// 다음글 링크
			$wr_next = array('href' => '', 'subject' => '다음글이 없습니다.', 'html' => '<a>다음글이 없습니다.</a>');
			if ($pn['next']['wr_id']) {
				$next_wr_subject = cut_str(get_text($pn['next']['wr_subject']), 255);
				$wr_next['href'] = RT_PATH."/board/".BO_TABLE."/view".qstr_rep($qstr, 'wr_id', $pn['next']['wr_id']);
				$wr_next['subject'] = $next_wr_subject;
				$wr_next['html'] = '<a href="'. $wr_next['href'] .'">'. $wr_next['subject'] .'</a>';
			}
		}
		...
		$result = $this->M_upload_files->get_files(BO_DB, $wr_id, 'uf_no,uf_editor,uf_source,uf_file,uf_type');
		foreach ($result as $row) {
			...
			$files[$row['uf_no']]['use_editor'] = $row['uf_editor'];
			$files[$row['uf_no']]['type'] = $row['uf_type'];
		...
		$vars = array(
			...
			'_CSS_'			=> array('board'),
			'_JS_'			=> array('board', 'jprintArea'),
			
			// 한글 깨짐시 cut_hangul_last 사용
