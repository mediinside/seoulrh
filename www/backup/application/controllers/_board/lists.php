	...
	function index($view=FALSE) {
		...
		if (!$sst) {
			if ($board['bo_sort_field'])
				list($sst, $sod) = explode(' ', $board['bo_sort_field']);
			else {
				$sst = 'wr_num, wr_reply';
				$sod = 'asc';
			}
        ...
        $config['first_link']		= '<img class="mR5" src="'. BO_IMG_PATH .'/btn_page_first.gif">';
        $config['next_link']		= '<img class="mL5" src="'. BO_IMG_PATH .'/btn_page_next.gif">';
        $config['prev_link']		= '<img class="mR5" src="'. BO_IMG_PATH .'/btn_page_prev.gif">';
        $config['last_link']		= '<img class="mL5" src="'. BO_IMG_PATH .'/btn_page_last.gif">';
		...
		foreach ($result as $i => $row) {
			...
			$row = get_convert($row, $board, IMG_DIR.'/board', $board['bo_subject_len'], $qstr, TRUE);
		...
		// RSS 버튼
        if($board['bo_use_rss']) {
			$button['rss'] = "<a href='".RT_PATH."/board/".BO_TABLE."/rss' target='_blank'><img src='".BO_IMG_PATH."/ico_rss.gif' title='RSS' alt='RSS'/></a>";
        }
		...
		// 관리자 체크박스 및 버튼 표시
		if(IS_ADMIN) {
			$button['chkbox'] = "<a href=\"javascript:select_delete('$qstr');\"><img src='".BO_IMG_PATH."/btn_select_delete.gif' title='선택삭제' alt='선택삭제'/></a> ";
			if (SU_ADMIN || IS_ADMIN == 'group') {
				$button['chkbox'] .= "<a href=\"javascript:select_copy('copy','$qstr');\" onfocus=\"blur();\"><img src='".BO_IMG_PATH."/btn_select_copy.gif' title='선택복사' alt='선택복사'/></a> ";
				$button['chkbox'] .= "<a href=\"javascript:select_copy('move','$qstr');\" onfocus=\"blur();\"><img src='".BO_IMG_PATH."/btn_select_move.gif' title='선택이동' alt='선택이동'/></a> ";
			}
		}
