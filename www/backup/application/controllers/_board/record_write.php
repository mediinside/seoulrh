...
class Record_write extends CI_Controller {
	...
	function update() {
		...
		if(!IS_MEMBER) {
			$config[] = array('field'=>'wr_name', 'label'=>'이름', 'rules'=>'trim|required|min_length[2]|max_length[10]');
			$pass_req = ($w == 'u') ? '' : '|required';
			$config[] = array('field'=>'wr_password', 'label'=>'비밀번호', 'rules'=>'trim'.$pass_req.'|min_length[4]|max_length[20]|md5');
		}
