...
class Layout extends CI_Controller {
	...
	function lists() {
		...
		foreach ($list as $i => $row) {
			...
			$list[$i]['s_pre'] = '';
			$list[$i]['s_mod'] = icon('수정', 'layout/form/u/'.$row['ly_id'].$qstr);
	...
	function form($w='', $id='') {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($w == '' || $w != 'u') {
				...
				$html_code = encodeVars(getCode($row['ly_file'], $this->input->server('DOCUMENT_ROOT') . LAYOUT_PATH));
		...
		else
		{
			...
			$ly_parameter = $this->input->post('ct_parameter');
			...
			$ly_css = is_array($this->input->post('ly_css')) ? implode('|', remove_empty($this->input->post('ly_css'))) : '';
			$ly_js = is_array($this->input->post('ly_js')) ? implode('|', remove_empty($this->input->post('ly_js'))) : '';
			
			$data = array(
					'ly_platform'	=> $this->input->post('ly_platform'),
					'ly_file'		=> $this->input->post('ly_file'),
					'ly_css'		=> $ly_css,
					'ly_js'			=> $ly_js,
					'ly_hidden'		=> $this->input->post('ly_hidden'),
					'ly_parameter'	=> param_encode($ly_parameter),
			...
			// code를 html 파일로 저장
			$fullPath = $this->input->server('DOCUMENT_ROOT') . LAYOUT_PATH . '/'. $this->input->post('ct_url');
			setCode($this->input->post('ly_file'), $ly_code, $ct['ly_file'], $fullPath);
			
			goto_url(ADM_F.'/layout/form/u/'.$id.$qstr);
		}
	}
	...
	function delete() {
		...
		foreach($ids as $i) {
			...
			$this->M_a_dbvars->delete('layout', $i);
			
			delCode($row['ly_file'], $this->input->server('DOCUMENT_ROOT') . LAYOUT_PATH);
		}
