...
class Boardextra extends CI_Controller {
	...
	function ex($bid='') {
		...
		$bo = $this->M_basic->get_board($bid, 'bid,bo_subject');
		if (!isset($bo['bid']))
			alert('존재하지 않는 게시판 입니다.');

		$table_name = 'ki_extra_'.$bid;

		$config = array(
			array('field'=>'bid', 'label'=>'아이디', 'rules'=>'trim|required|max_length[20]|alpha_dash'),
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$vars = array(
				'_TITLE_'		=> $bo['bo_subject'].' 여분필드 관리',
				'_BODY_'		=> ADM_F.'/board/boardextra',
			...
			goto_url(ADM_F.'/boardextra/ex/'.$bid);
		}
	}
}
?>
