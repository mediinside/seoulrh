...
class Modify extends CI_Controller {
	...
	function __construct() {
		...
		$this->conf = $this->M_mb_infor->getConfig('mcf_modify_param');
	}
	...
	function _form($member) {
        ...
        $mail_chk = ($member['mb_mailling']) ? "checked='checked'" : FALSE;
		$open_chk = ($member['mb_open']) ? "checked='checked'" : FALSE;
		...
		$vars = array_merge( array(
			...
			'_JS_'			=> array('md5','kcaptcha', 'jvalidate', 'jvalid_ext', 'jvalid_reg', 'jquery-ui.min', 'jtimepicker'),
	...
	function password() {
		...
		$member = unserialize(MEMBER);
		$conf = $this->M_mb_infor->getConfig('mcf_password_param');
		
		$mb_password = ($this->session->userdata('ss_tmp_password')) ? $this->session->userdata('ss_tmp_password') : md5($this->input->post('mb_password'));
		if ($this->encrypt->decode($member['mb_password']) != $mb_password)
			alert('비밀번호가 맞지 않습니다.', 'member/confirm/qry/member.modify.password');

		$vars = array_merge( array(
			'_TITLE_'		=> '비밀변호 변경',
			'_BODY_'		=> 'member/modify_password',
			'_JS_'			=> array('md5','kcaptcha', 'jvalidate'),
			
			'IMG_PATH'		=> IMG_DIR .'/member'
		), $member, $conf['password_param']);
	...
	function password_update() {
		$config = array(
			...
			array('field'=>'old_password', 'label'=>'현재 비밀번호', 'rules'=>'trim|required|min_length[4]|max_length[20]|md5'),
