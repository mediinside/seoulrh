...
class Join extends CI_Controller {
	...
	function __construct() {
		...
		$this->conf = $this->M_mb_infor->getConfig('mcf_privacy, mcf_stipulation, mcf_join_param');
	}
	...
	function index() {
		...
		if($this->config->item('cf_use_jumin')) {
			$config = array(
					array('field'=>'mb_name', 'label'=>'이름', 'rules'=>'trim|required|min_length[2]|max_length[10]'),
					array('field'=>'mb_jumin', 'label'=>'주민등록번호', 'rules'=>'trim|required|exact_length[13]|is_natural_no_zero')
			);
		...
		if(!$this->input->post('agree') || !$this->input->post('agree2') || !$jumin_pass) {			
			$vars = array_merge( array(
				...
				'privacy'		=> $this->conf['mcf_privacy'],
				'stipulation'	=> $this->conf['mcf_stipulation'],
				'use_jumin'		=> $this->config->item('cf_use_jumin')
			), $this->conf['join_param']);
	...
	function _form() {
		...
		$vars = array_merge( array(
			...
			'_JS_'			=> array('md5', 'kcaptcha', 'jvalidate', 'jvalid_ext', 'jvalid_reg', 'jtimepicker'),
	...
	function update() {
		...
		$config = array(
			...
			array('field'=>'mb_password', 'label'=>'비밀번호', 'rules'=>'trim|required|min_length[4]|max_length[20]|md5'),
