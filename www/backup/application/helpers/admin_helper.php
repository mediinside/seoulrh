...
function icon($act, $link='', $target='_self') {
	$img = array('입력'=>'insert', '추가'=>'insert', '생성'=>'insert', '수정'=>'modify', '삭제'=>'delete', '보기'=>'view', '미리보기'=>'view', '복사'=>'copy', '로그인'=>'login');
...
function get_layout_select($name, $slt_id='', $not_id='') {
	$CI =& get_instance();
	$CI->load->model(ADM_F .'/M_a_layout');
...
// 해당 디렉토리의 하위 디렉토리 반환 
function get_files($path, $is_dirs=FALSE, $key_number=FALSE) {
	$CI =& get_instance();
	...
	if($map) {
		foreach($map AS $dir => $file) {
			if(is_array($file) && $is_dirs) {
				if($key_number) $list[] = $dir;
				else $list[$dir] = $dir;
			}
			else if(!is_array($file) && !$is_dirs) {
				if($key_number) $list[] = $file;
				else $list[$file] = $file;
			}
		}
		if($key_number) sort($list);
		else ksort($list);
	}
...
// 배송사 셀렉트
function sel_deliverer($colName, $selected) {
	$CI =& get_instance();
	$dlv_list = $CI->M_shop->deliverer;

	$HTML = '<select id="'.$colName.'" name="'.$colName.'">' ."\n";
	$HTML .= '<option value="">선택</option>' ."\n";
	foreach($dlv_list AS $no => $dlv) {
		$sel = ($no == $selected) ? 'selected="selected"' : '';
		$HTML .= '<option value="'. $no .'" '. $sel .'>'. $dlv['name'] .'</option>' ."\n";
	}
	$HTML .= '</select>' ."\n";
