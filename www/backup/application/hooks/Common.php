...
class _Common {
	function index() {
		...
		$CI->M_config->getConfig('cf_title,cf_webmaster,cf_domain,cf_mobile');
		
		$is_member = $is_super = $is_mobile = FALSE;
		...
		if($CI->config->item('cf_mobile')) {
			$CI->load->library('user_agent');
			if($CI->agent->is_mobile() || $CI->uri->segment(1) == 'mobile') {
				$is_mobile = TRUE;
			}
		}
