...
(function($, undefined) {
	...
	$.fn.ix_rolling = function(settings) {
		...
		return this.each(function() {
			...
			if (options.template == 'twitter' && options.userid) {
				$.getJSON('https://api.twitter.com/1/users/show.json?screen_name=' + options.userid + '&include_entities=false&callback=?', function(results) {
					...
					_html += '<img class="s-profile-img" align="left" src="https://api.twitter.com/1/users/profile_image?screen_name=' + options.userid + '&size=mini " title="' + options.userid + '"/></a>';
			...
			var sync_count = 0, now_sync = $("").appendTo($$).toggle(function() {
			...
			var reload = function() {
				...
				$.ajax({
					url : url + (!options.userid ? "&since_id=" + $$.data('recently_max_id') : ""),
					type : "GET",
					cache : false,
