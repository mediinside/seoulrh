if (typeof(COMMON_JS) == 'undefined') {
	...
	// 이메일 수동입력 필드 show/hide
    function email_manual(jobj, val)
	{
    	var select = jobj;
    	var account = select.prev();
    	var domain = select.next();
    ...
    // 이메일 주소 병합
    function email_merge(jobj)
	{
    	var select = jobj;
    	var account = select.prev();
    	var domain = select.next();
    	var hidden = select.prev().prev();
    	var domain_str = select.val() && select.val() != '0' ? select.val() : domain.val();
    	
    	hidden.val(account.val() +'@'+ domain_str);
	}
    ...
    function importScript(FILES) {
        var _importScript = function(filename) { 
        	if (filename) {
        		document.write('<script type="text/javascript" src="'+rt_path+'/src/js/'+filename+'.js"></s'+'cript>');
            }
        };
	...
	function add_child(in_obj, url, jsonParam) {
		if(jsonParam == undefined) jsonParam = '';

		$(in_obj).append('<li style="margin-bottom:5px;"></li>');
		var idx = $(in_obj +' li:last').index();
		...
		$.post(url, jsonParam,
			function(htmlData) {
				...
				else if(htmlData == '200') {
					alert('자료가 없습니다.');
				}
				else {
					var _html =	htmlData +' <input type="button" class="btn_simp" value=" 삭제 " onclick="remove_child($(this));"/>';
					
					$(in_obj +' li').eq(idx).append(_html);
				}
		})
	}
