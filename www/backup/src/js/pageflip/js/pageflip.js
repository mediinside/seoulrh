...
// initial spread, presume we usually start on page 1, but not necessarily
	page1 = $(".pageContent:eq(" + parseInt(pageIndex) + ")");
	...
	if( $(".pageContent").length > parseInt(pageIndex + 2) ) { // if there's a next page...
		page3 = $(".pageContent:eq(" + parseInt(pageIndex + 2 + pageGap) + ")");
		...
		page4 = $(".pageContent:eq(" + parseInt(pageIndex + 3 + pageGap) + ")");
	...
	*/
	...
	//if(pageIndex > 1) { // there should be a previous page
		...
		page0 = $(".pageContent:eq(" + parseInt(pageIndex - 1 - pageGap) + ")");
		...
		pageN1 = $(".pageContent:eq(" + parseInt(pageIndex - 2 - pageGap) + ")");
...
function nextPage() {
	...
	if(jump)
		pageGap = (jump + (jump % 2)) - pageIndex - 4;
	else
		pageGap = 0;
