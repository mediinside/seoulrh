if (typeof(CATEGORY_JS) == 'undefined') {
    ...
    function changeCate(c, lo, qstr) {
        ...
        if (is_ready && lo) {
			...
			document.location.replace(rt_path + '/' + ca_url + cval.replace('-', '.') + qstr); 
			return false;
		}
		...
		if (document.getElementById(sid) == null) {
			var disabled = c.attr('disabled') == 'disabled' ? "disabled='disabled'" : "";
			var first = (lo) ? "<option value='"+cval+"'>전체</option>" : "<option value=''>선택하세요</option>";
			cpar.find('select').removeAttr('name');
			c.after("<select id='"+sid+"' name='"+cname+"' onchange='javascript:changeCate(this, "+lo+", \""+qstr+"\");' "+ disabled +">"+first+"</select>");
		}
		else
