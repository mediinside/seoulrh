...
class Record_movecopy extends CI_Controller {
	...
	function update() {
		...
		$board = $this->M_basic->get_board($bid, 'bo_subject');
		$member = unserialize(MEMBER);
		$base_url = $this->config->item('base_url');
		
		// 원본 파일 디렉토리
		$ori_dir = DATA_PATH.'/file/'.$bid;
		
		$save = array(); // 이동시 삭제를 위한
		$save_count_write = 0; // 수행한 전체 글 개수
		$save_count_comment = 0; // 수행한 전체 코멘트 개수
		
		$wr_nums = $this->M_board_mvcp->get_dist_num($bid, $wr_ids);
		
		// 해당 게시물의 답글까지 모두 가져와
		$result =  $this->M_board_mvcp->get_write_num($bid, $wr_nums);
		...
		foreach ($bids as $move_bid) {
			...
			foreach ($result as $cnt => $wr) {
				...
				// 해당 게시판으로 글 Insert					
          		...
          		// 관련글 복사
          		$this->M_postlink->post_copy($bid, $wr['wr_id'], $move_bid, $insert_id);
	            ...
	            if (!$wr['wr_is_comment']) {
	            	...
	            	if ($wr['uf_count_image'] > 0 || $wr['uf_count_file'] > 0) {
		            	// 해당 게시물의 파일 정보를 가져와
		            	$result3 = $this->M_board_mvcp->get_write_file($bid, $wr['wr_id']);
						...
						$this->M_board_mvcp->content_update($move_bid, $insert_id, str_replace(
							array($base_url.'/data/file/'.$bid,
								  $base_url.'/board/'.$bid.'/download/wr_id/'.$wr['wr_id']),
		...
		if($sw == 'move') {
			...
			$this->M_dbvars->ref_move($bid, $wr_re_co_ids, $move_bid, $insert_ids);
			
			// 이동된 글이 관련글로 되어있는 글들의 link_id값을 이동된 id값으로 변경
			$this->M_postlink->post_move($bid, $wr_re_co_ids, $move_bid, $insert_ids);
			
			// 삭제될 원글의 관련글 삭제
			$this->M_postlink->post_delete($bid, $wr_re_co_ids);
			
			// 원글 삭제
			$this->M_board_mvcp->write_delete($bid, $wr_re_co_ids);
			
			$bo_min_wr_num = $this->M_board_mvcp->get_min_num($bid);
			$this->M_board_mvcp->bo_count_update($bid, $save_count_write, $save_count_comment, $bo_min_wr_num, '-');
		}
