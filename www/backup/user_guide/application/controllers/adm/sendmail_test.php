...
class Sendmail_test extends CI_Controller {
	...
	function index() {
		...
		if ($this->input->post('mail_addr')) {
			...
			$this->email->clear();
			$this->email->from('test@test.com', '메일검사');
			$this->email->to($mail_addr);
			$this->email->subject($subject);
			$this->email->message($content);
			if (!$this->email->send())
				$mail_msg = '<br/><b>※ 메일전송 오류</b><br/>'.$this->email->print_debugger();
			else
				$mail_msg = '<br/><b>'.$mail_addr.'</b> (으)로 메일을 발송 하였습니다.<br/>해당 주소로 메일이 왔는지 확인하여 주십시오.<br/>메일이 오지 않는다면 프로그램의 오류가 아닌 메일 서버(sendmail)의 오류일 가능성이 있습니다.<br/>이런 경우에는 웹 서버관리자에게 문의하여 주십시오.';
		}

		$vars = array(
			'_TITLE_'		=> '메일전송 테스트',
			'_BODY_'		=> ADM_F.'/sendmail_test',
