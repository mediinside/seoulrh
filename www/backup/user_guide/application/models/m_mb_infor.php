...
class M_mb_infor extends CI_Model {
	...
	function is_jumin_name() {
		...
		$query = $this->db->get_where('ki_member', array('mb_jumin' => md5(sha1($this->input->post('mb_jumin')))));
	...
	function leave() {
		...
		$this->db->where('mb_id', $this->input->post('mb_id'));
		$this->db->where('mb_password', $this->encrypt->decode($this->input->post('mb_password')));
		$this->db->update('ki_member', $sql);
	}
}
?>
