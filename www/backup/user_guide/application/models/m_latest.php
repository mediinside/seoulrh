...
class M_latest extends CI_Model {
	...
	// 최신글
    function write($bid='', $limit=5, $cut=50, $orderby='wr_datetime desc', $tbl_not_in=FALSE, $thumb=FALSE) {
        $this->db->cache_on();
        ...
        if($limit) {
        	$thumbnail = $thumb ? '/useful/thumbnail/'.$thumb['w'].'x'.$thumb['h'].'/'.$bid : '';
	        $list = array_fill(0, $limit,
						array_combine(
							array('href','subject','content','comt_cnt','thumb','date'),
		...
		foreach($result as $i => $row) {
			...
			if($thumb) {
				$videoThumb = videoThumbnail($row['wr_content']);
				if(isset($thumb['w']) && isset($thumb['h']) && $row['uf_count_image']) {
					$list[$i]['thumb'] = '/useful/thumbnail/'.$thumb['w'].'x'.$thumb['h'].'/'.$row['bid'].'/'.$row['wr_id'];
				}
				else if($thumb && $videoThumb) {
					$list[$i]['thumb'] = $videoThumb;
				}
			}
