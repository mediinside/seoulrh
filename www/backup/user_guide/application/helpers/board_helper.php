...
function sns_post($bid, $wr_id, $title, $content) {
	...
	$str = '';
	$str .= "<a href='http://twitter.com/home?status=".$title."%0A".$url."' target='_blank'><img src='".BO_IMG_PATH."/btn_twitter.png' title='트위터' alt='트위터'/></a>";
	$str .= " <a href='http://www.facebook.com/sharer.php?s=100&p[url]=".$url."&p[images][0]=".$img."&p[title]=".$title."&p[summary]=".$content."' target='_blank'><img src='".BO_IMG_PATH."/btn_facebook.png' title='페이스북' alt='페이스북'/></a>";
	$str .= " <a href='http://me2day.net/posts/new?new_post[body]=%22".str_replace('%22', '%5C%22', $title)."%22:".$url."%0A%0A".$url."&new_post[tags]=' target='_blank'><img src='".BO_IMG_PATH."/btn_me2day.png' title='미투데이' alt='미투데이'/></a>";
	$str .= " <a href='http://csp.cyworld.com/bi/bi_recommend_pop.php?url=".$url."&title=".urlencode(base64_encode($title_de))."&thumbnail=".$img."&summary=".urlencode(base64_encode($content_de))."&writer=' target='_blank'><img src='".BO_IMG_PATH."/btn_cy.png' title='C공감' alt='C공감'/></a>";
...
function setBoardDb($boards) {
	...
	if(!isset($boards[0]) || !is_array($boards[0])) {
		$boards = array($boards);
	}
...
function bo_list_where($board, $is_admin=FALSE) {
	...
	switch($board['bo_where']) {
		case 'mb_id' :
			if(!$is_admin) {
				$CI =& get_instance();
				$wr_ids = $CI->M_board->my_wr($member['mb_id']);
