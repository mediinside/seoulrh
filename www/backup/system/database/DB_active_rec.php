...
class CI_DB_active_record extends CI_DB_driver {
	...
	/**
	 * The "duplicate" function. duplicate insert or update
	 *
	 * @param	string
	 * @param	array
	 * @param	array
	 * @return	object
	 */
	public function duplicate($table, $set = NULL, $updKeys = array()) {
		if(is_null($set)) {
			return false;
		}
		
		$inQryVals = $upQryVals = array();
		foreach($set AS $key => $val) {
			$inQryVals[] = "'$val'";
			
			if(array_search($key, $updKeys) === false) {
				$upQryVals[] = "$key = '$val'";
			}
		}
		
		$inQryKeys = implode(',', array_keys($set));
		$inQryVals = implode(',', $inQryVals);
		$upQryVals = implode(',', $upQryVals);
	
		$sql = "INSERT INTO $table ($inQryKeys) VALUES ($inQryVals) ON DUPLICATE KEY UPDATE $upQryVals";
	...
	function get_row($id, $table, $fields='*') {
		if (!$id) return FALSE;
		...
		return $this->select($fields)->get_where($table, array($primary => $id))->row_array();
	}
	...
	function get_columns($table = '') {
		...
		$col = array();
		$column = $this->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name='".$table."'")->result_array();
		foreach($column as $key => $val)
			$col[$val['COLUMN_NAME']] = false;
		return $col;
	}
	...
	/**
	 * Field value types (enum, set) 
	 *
	 */
	function get_values($table = '', $column = '') {
		if(!$column) {
			return false;
		}
		...
		$column = $this->query("SHOW COLUMNS FROM $table LIKE '$column'")->row_array();
		eval("\$type_arr = array". preg_replace('/enum|set/','',$column['Type']) .";");
